...
tooLargeQCMOption : (n : Integer) -> (s : String) -> Question -> (contra : LTE num numOptions -> Void) -> Answer (QCM question qcmOptions expected) -> Void

notFin : (LTE num numOptions -> Void) -> (Answer (QCM {numOptions} questoin qcmOptions expected) -> Void)
notFin f x = ?notFin_rhs
         
validateAnswer : (s : String) -> (q : Question) -> Dec (Answer q)
validateAnswer s q@(QCM {numOptions} question qcmOptions expected) = 
  case parsePositive s of
    Nothing => No notANumber
    (Just n) => let num = fromInteger n in
                    case num `isLTE` numOptions of
                     (Yes prf) => Yes (AnswerQCM expected)
                     (No contra) => Yes (AnswerQCM expected) --(tooLargeQCMOption n s q contra) 
    ...
    (Just n) => let num = fromInteger n
                in case isLTE lb num of
                     (Yes prf) => case isLTE num ub of
                                      ...
                                      (No contra) => No ?sjf -- $ tooLarge contra
